...
/* contract Vin_Token 
{
    string public MyTokenName = "Lakshmi";
    string public Symbol = "Runa";
    uint256 public Decimal = 18;
    uint256 public TotSupply = 1000000000000000000000; //1000 Million supply 
} */
...
/* contract Vin_Token 
{
    ...
    event Transfer_Event(address indexed from, address indexed to, uint256 value);

    constructor(string memory _MyToken, string memory _Symbol, uint _decimal, uint _TotSupply)
    {
        MyTokenName = _MyToken;
        Symbol = _Symbol;
        Decimal = _decimal;
        TotSupply = _TotSupply;
        BalanceOff[msg.sender] = TotSupply;
    }

    function TransferToken(address _to, uint256 _value) external returns (bool success)
    {
        require(BalanceOff[msg.sender] >= _value);
        BalanceOff[msg.sender] = BalanceOff[msg.sender] - (_value);
        BalanceOff[_to] = BalanceOff[_to] + (_value);
        emit Transfer_Event(msg.sender, _to, _value);
        return true;
    }
} */
